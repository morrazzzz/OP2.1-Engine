...
sub state_read {
	$_[1]->unpack_properties($_[0], properties_info);
	#расскомментить строки ниже для подсчета максимального значения
	# if ($_[0]->{'spawn_story_id'}>$all_spawn::start_spawn_story_id) #наибольшее   start_spawn_story_id
	# {
		# $all_spawn::start_spawn_story_id=$_[0]->{'spawn_story_id'};
	# }
	# if ($_[0]->{'story_id'}>$all_spawn::start_story_id) #наибольшее start_story_id
	# {
...
sub state_export {
	#если не установлено ни одно из требуемых переменных 
	#-1 значение по умолчанию при отсутствии id
...
sub state_export {
	...
	if (($_[0]->{'spawn_story_id'}==-1) and ($_[0]->{'story_id'}==-1)) 
	{
		if ($_[0]->{'section_name'} eq 'zone_flame_small') #только нужная нам секция
		{
			$all_spawn::start_spawn_story_id++;
			$all_spawn::start_story_id++;
			$_[0]->{'spawn_story_id'}=$all_spawn::start_spawn_story_id;
